...
- **Milestones are not a separate entity.** A milestone is a `TASKS` entry where `start === end` (a same-day task) — it has a lane, status, owner, and description exactly like any other task, created through the same Task form. The only difference is visual: it renders as a diamond marker instead of a bar. **A milestone can also be perched above or below its lane row** (`milestonePosition`) with a connector line back to its actual date, instead of always sitting directly on the row — added because the default on-row placement gets visually cramped next to other tasks.
...
- **Chart title and x-axis label are editable through the Configure screen, not just hand-edited constants.** Every other piece of roadmap content (PIs, Sprints, Lanes, Tasks) is edited via Configure; title/axis-label followed the same pattern once that inconsistency was noticed. Both persist through Open/Save like the rest of the roadmap data, with safe fallback defaults (`"Product Roadmap"` / `"Date"`) for older saved files missing these keys.
- **A custom `vizimap` theme is the default**, alongside the ten built-in Plotly themes, all selectable from Chart Settings. The first custom-theme pass (2026-08-26) was found too conservative on review — round 2 (filed as #61) covers bolder typography, a real segmented-control look for the Chart/Configure toggle, styled row labels, and a non-flat-white background, all within the existing no-new-dependencies constraint (system font stack only, no CDN font).
...
- Plotly.js via CDN script tag, pinned to a specific version (`2.35.2` as of this writing) — don't float on `latest`. **No other CDN requests or dependencies are permitted** — this has been reaffirmed explicitly (2026-08-26) specifically to rule out adding a webfont service for typography improvements; use a strong system-font stack instead.
...
- Gantt bars use the standard Plotly technique: `type: 'bar', orientation: 'h'`, date-typed x-axis, `base` = start date, `x` = duration. A task with `start === end` (a milestone) renders instead as a scatter-trace point with `marker.symbol: 'diamond'`, positioned in its lane's row (or offset above/below per `milestonePosition`) like any other task in that lane.
...
- No bundler, no npm install step, and no additional CDN requests of any kind (including webfonts) beyond the one existing Plotly script tag. If a library is ever needed beyond Plotly, it comes from a CDN `<script>` tag as a deliberate, discussed exception — not by default.
