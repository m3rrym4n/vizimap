...
- **Task color is chosen manually, per task — not derived from status.** Corrected design, 2026-08-26: the original status-driven legend/coloring from #6 was removed as unhelpful in practice. Each task has its own `color` field, set via a color picker in the Task form. `status` still exists as a field (visible via hover and the click-to-overlay dialog) but no longer drives any visual color or legend.
- **Tasks carry a longer freeform description, with two separate ways to see it.** (1) The hover tooltip stays a quick-glance summary (name/date(s)/status/owner). (2) Clicking a task shows an on-chart overlay with the full description — both of these are screen-only and disappear on PNG export. (3) **`descriptionPosition`** (`'hidden' | 'above' | 'below'`, default `'hidden'`) bakes the description as a static, always-visible chart annotation above or below the task's bar/marker — added 2026-08-26 specifically because hover and the click-overlay both vanish in an exported PNG, and Stan needs descriptions visible in the exported image for leadership briefs.
...
- **Two distinct view modes: Chart and Configure.** A toggle at the top of the page shows either the chart (plus PNG export) or the editing/configuration UI (PI/Sprint/Lane/Task forms and lists, plus Open/Save) — never both at once.
- **Exportable to PNG** for dropping directly into briefs/slide decks (see #13). The chart uses the full available viewport width (no fixed max-width) and its height scales with lane count, added 2026-08-26 after the original fixed small size proved too cramped in real use.
- **Chart title is a hand-editable constant** (`CHART_TITLE`, declared alongside the data collections), not hardcoded inside the render function — added 2026-08-26 so it's editable in the same place as the roadmap data itself, no UI control for it.
...
- **No legend.** Removed 2026-08-26 along with status-driven coloring — it was static and unhelpful. Task color is per-task and manual (see above); don't reintroduce a legend without a specific reason to.
